...
type ViewName = 'Today' | 'Journey' | 'Calendar' | 'Progress' | 'Members';
const views: ViewName[] = ['Today', 'Journey', 'Calendar', 'Progress', 'Members'];
...
function dayDifference(left: Date, right: Date) {
  return Math.round((Date.UTC(left.getFullYear(), left.getMonth(), left.getDate()) - Date.UTC(right.getFullYear(), right.getMonth(), right.getDate())) / 86400000);
}
...
export default function ConflictJourneyScreen() {
  ...
  async function openReading(kind: 'bible' | 'commentary') {
    if (!data) return;
    const url = kind === 'bible' ? reading.bibleUrl : reading.commentaryUrl;
    if (!url) return;
    if (session) {
      const previous = progressMap.get(reading.id) ?? { reading_id: reading.id, bible_complete: false, commentary_complete: false };
  ...
  const todayProgress = progressMap.get(reading.id);
  return <View style={[styles.page, { paddingTop: insets.top }]}>
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      {view === 'Today' ? <TodayView canSave={Boolean(session)} onSignIn={authenticate} reading={reading} index={currentIndex} progress={todayProgress} principles={data.principles.filter((item) => item.reading_id === reading.id)} nextPrinciple={data.principles.reduce((max, item) => Math.max(max, item.principle_number), 0) + 1} completed={completed} principleBody={principleBody} crossRefs={crossRefs} busy={busy} onPrincipleBody={setPrincipleBody} onCrossRefs={setCrossRefs} onSavePrinciple={savePrinciple} onOpen={openReading} onToggle={toggleProgress} onPrevious={() => goToReading(currentIndex - 1)} onNext={() => goToReading(currentIndex + 1)} /> : null}
      {view === 'Journey' ? <JourneyView progressMap={progressMap} openBook={openBook} setOpenBook={setOpenBook} goToReading={goToReading} /> : null}
      {view === 'Calendar' ? <CalendarView canSave={Boolean(session)} onSignIn={authenticate} data={data} progressMap={progressMap} month={calendarMonth} setMonth={setCalendarMonth} onSettings={(settings) => { setData({ ...data, settings }); }} userId={session?.user.id} goToReading={goToReading} /> : null}
...
function TodayView(props: { canSave: boolean; onSignIn: () => void; reading: ConflictReading; index: number; progress?: ConflictProgress; principles: ConflictPrinciple[]; nextPrinciple: number; completed: number; principleBody: string; crossRefs: string; busy: boolean; onPrincipleBody: (value: string) => void; onCrossRefs: (value: string) => void; onSavePrinciple: () => void; onOpen: (kind: 'bible' | 'commentary') => void; onToggle: (field: 'bible_complete' | 'commentary_complete') => void; onPrevious: () => void; onNext: () => void }) {
...
function CalendarView({ canSave, onSignIn, data, progressMap, month, setMonth, onSettings, userId, goToReading }: { canSave: boolean; onSignIn: () => void; data: ConflictMemberData; progressMap: Map<string, ConflictProgress>; month: Date; setMonth: (date: Date) => void; onSettings: (settings: ConflictMemberData['settings']) => void; userId?: string; goToReading: (index: number) => void }) {
  ...
  for (let day = 1; day <= days; day += 1) cells.push(new Date(month.getFullYear(), month.getMonth(), day, 12));
  async function mode(schedule_mode: 'pace' | 'calendar') { if (!userId) { onSignIn(); return; } const saved = await saveConflictSettings(userId, { ...data.settings, schedule_mode }); onSettings(saved); }
  return <><Eyebrow>A CALM VIEW OF THE DAYS</Eyebrow><Text style={styles.title}>Calendar</Text><Text style={styles.subtitle}>Your current start date is {data.settings.start_date}. Change it on the website for an exact date picker.</Text>{canSave ? <View style={styles.modeRow}><Pressable onPress={() => mode('pace')} style={[styles.mode, data.settings.schedule_mode === 'pace' && styles.modeActive]}><Text style={styles.modeText}>My pace</Text></Pressable><Pressable onPress={() => mode('calendar')} style={[styles.mode, data.settings.schedule_mode === 'calendar' && styles.modeActive]}><Text style={styles.modeText}>Calendar mode</Text></Pressable></View> : <OutlineButton title="Sign In to Save Calendar Settings" onPress={onSignIn} />}<Card><View style={styles.monthHeader}><Pressable onPress={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1, 12))}><Text style={styles.gold}>‹</Text></Pressable><Text style={styles.cardTitle}>{month.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</Text><Pressable onPress={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1, 12))}><Text style={styles.gold}>›</Text></Pressable></View><View style={styles.calendarGrid}>{['S','M','T','W','T','F','S'].map((label, index) => <Text key={`${label}-${index}`} style={styles.weekday}>{label}</Text>)}{cells.map((date, index) => { if (!date) return <View key={`empty-${index}`} style={styles.calendarCell} />; const readingIndex = dayDifference(date, parseISO(data.settings.start_date)); const item = conflictPlan.readings[readingIndex]; const done = item ? isConflictReadingComplete(item, progressMap.get(item.id)) : false; return <Pressable key={isoDate(date)} disabled={!item} onPress={() => goToReading(readingIndex)} style={[styles.calendarCell, done && styles.calendarDone]}><Text style={[styles.calendarNumber, !item && styles.muted]}>{date.getDate()}</Text>{item ? <Text style={styles.calendarDay}>D{item.day}</Text> : null}</Pressable>; })}</View></Card></>;
}
...
const styles = StyleSheet.create({
  ...
  dayNav:{flexDirection:'row',alignItems:'center',justifyContent:'space-between',gap:8},percent:{color:colors.gold,fontWeight:'900'},scriptureCard:{backgroundColor:colors.plum,padding:22},scriptureRef:{color:colors.text,fontSize:31,fontWeight:'900',lineHeight:37,marginBottom:12},review:{color:'#ffb9b3',fontSize:11,lineHeight:17,marginTop:14},checkRow:{flexDirection:'row',alignItems:'center',gap:10,marginTop:14},checkBox:{width:27,height:27,borderRadius:8,borderWidth:1,borderColor:colors.gold,alignItems:'center',justifyContent:'center'},checkBoxDone:{backgroundColor:colors.green,borderColor:colors.green},checkMark:{color:colors.charcoal,fontWeight:'900'},checkLabel:{color:colors.ivory,fontSize:12,fontWeight:'800'},
  ...
  modeRow:{flexDirection:'row',gap:8},mode:{padding:10,borderRadius:10,backgroundColor:colors.panel},modeActive:{backgroundColor:colors.plum2},modeText:{color:colors.ivory,fontSize:10,fontWeight:'800'},monthHeader:{flexDirection:'row',alignItems:'center',justifyContent:'space-between'},calendarGrid:{flexDirection:'row',flexWrap:'wrap',marginTop:15},weekday:{width:'14.285%',color:colors.muted,textAlign:'center',fontSize:9,fontWeight:'900',paddingVertical:7},calendarCell:{width:'14.285%',aspectRatio:.88,borderWidth:1,borderColor:colors.border,alignItems:'center',justifyContent:'center'},calendarDone:{backgroundColor:'rgba(79,193,139,.25)'},calendarNumber:{color:colors.text,fontSize:12,fontWeight:'800'},calendarDay:{color:colors.gold,fontSize:8,marginTop:3},

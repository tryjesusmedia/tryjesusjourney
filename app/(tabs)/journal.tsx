...
import { addGuestJournalEntry, getGuestJournal, removeGuestJournalEntry, type GuestJournal } from '@/lib/localStore';
...
export default function JournalScreen(){
  ...
  const load=useCallback(async()=>{setEntries(await getGuestJournal());},[]);
  useFocusEffect(useCallback(()=>{load();},[load]));
  async function save(){if(!body.trim()) return; const next:GuestJournal={id:String(Date.now()),title:title.trim()||'Reflection',body:body.trim(),createdAt:new Date().toISOString()};setEntries(await addGuestJournalEntry(next)); setTitle('');setBody('');setOpen(false);}
  async function remove(item:Entry){setEntries(await removeGuestJournalEntry(String(item.id)));}
